...
class TimeSheetRow extends Component<TimeSheetRowProps, TimeSheetRowState> {
  ...
  renderRows() {
    ...
    return rowData.map((item, index) => {
      // Check if previous rowData items have the same task name
      const previousItems = rowData.slice(0, index);
      const showTime = !previousItems.some((prevItem) => prevItem.taskName === item.taskName);
              ...
              {arr.map((num, index) => {
                ...
                if (currentDate === matchDate) {
                  return (
                    <td key={index}>
                      <InputGroup size="sm">
                        <Form.Control type="number" placeholder="0" value={item.time} />
